...
USE_REALSENSE = True
SOURCE = 2  # webcam ID or image path
START_CONF = 0.6
CSV_OUTPUT = "book_width_dataset/labels.csv"
FIXED_DISTANCE_MM = 400.0
DISPLAY_SCALE = 0.75
INFO_PANEL_WIDTH = 420
CENTER_ZONE_RATIO = 0.30
...
detections = []
selected_det = None
USE_DEPTH_AWARE_MM = False
intrinsics = None
display_resize_scale = 1.0
WAITING_FOR_INPUT = False
INPUT_VALUES = None

os.makedirs("book_width_dataset/images", exist_ok=True) #Folder for images
...
def input_worker():
    ...
    try:
        ...
        weight_g = float(input("Enter Weight (g): "))
        INPUT_VALUES = (D, T, H, W, weight_g)
    except:
        INPUT_VALUES = None
    WAITING_FOR_INPUT = False
...
while True:
    ...
    if not WAITING_FOR_INPUT and INPUT_VALUES is not None:
        D, T, H, W, weight_g = INPUT_VALUES

        # Calibrate depth automatically using regression
        k_const = 0.9799
        b_const =  2.29 
        depth_raw = PENDING_DET["depth_mm"] if PENDING_DET["depth_mm"] else FIXED_DISTANCE_MM
        depth_calibrated_mm = k_const * depth_raw + b_const

        # Use calibrated depth for thickness/height estimation
        thickness_mm_cal = PENDING_DET["thickness_px"] * depth_calibrated_mm / (intrinsics.fx if intrinsics else 600)
        height_mm_cal    = PENDING_DET["height_px"] * depth_calibrated_mm / (intrinsics.fy if intrinsics else 600)
        ...
        row = {
            "filename": img_name,
            "D": D,
            "depth_rs_mm": depth_raw,
            "depth_calibrated_mm": depth_calibrated_mm,
            "thickness_true_mm": T,
            "height_true_mm": H,
            "W": W,
            "weight_g": weight_g,
            ...
            "thickness_mm_est": thickness_mm_cal,
            "height_mm_est": height_mm_cal,
            "mode": PENDING_DET["mode_used"]
        }
        df = pd.DataFrame([row])
        df.to_csv(CSV_OUTPUT, mode="a", header=not os.path.exists(CSV_OUTPUT), index=False)

        print(f"💾 Saved {img_name} | Depth Calibrated={depth_calibrated_mm:.2f} mm | D={D} | T={T} | H={H} | W={W} | Book Type={book_type}")
    ...
    if results.obb:
        for box, conf, cls in zip(results.obb.xywhr, results.obb.conf, results.obb.cls):
            ...
            if depth_ok:
                depth_calibrated_mm = 0.9799 * depth_mm + 2.29
                thickness_mm = px_t * depth_calibrated_mm / intrinsics.fx
                height_mm = px_h * depth_calibrated_mm / intrinsics.fy
    ...
    if selected_det:
        lines=[
            ...
            f"Depth Calibrated : {selected_det['depth_calibrated_mm']:.1f} mm" if selected_det['depth_calibrated_mm'] else "Depth Calibrated : N/A",
